...
class Mahjong:
    ...
    def __calculate_fan(self, player: int) -> int:
        claimings, tiles = self.players_data[player].claimings_and_tiles  # ??, ??
        win_tile = self.last_tile  # ??
        flower_count = 0  # ???
        is_self_draw = player == self.round_stage  # ??
        is_fourth_tile = self.played_tiles[self.last_tile] == 3  # ??
        is_kong = self.last_meld_kung or self.last_concealed_kung or self.current_meld_kung  # ?
        is_last_tile = len(self.players_data[(self.round_stage + 1) % consts.NUM_PLAYERS].tile_wall) == 0  # ??????
        player_wind = player  # ??
        round_wind = self.round_wind  # ??
