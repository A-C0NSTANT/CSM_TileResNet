...
def add_PLAY(res, index, tile):
    res['actions'][index].append([ACTION['??'], tile])
    add_PASS(res, index)


def add_CHOW(res, index, tile, new_tile):
    res['actions'][index].append([ACTION['?'], f'{tile} {new_tile}'])
    add_PASS(res, index)


def add_PENG(res, index, new_tile):
    res['actions'][index].append([ACTION['?'], new_tile])
    add_PASS(res, index)
...
def add_BUGANG(res, index):
    res['actions'][index].append([ACTION['??']])
    add_PASS(res, index)
...
def preprocess_file(lines):
    ...
    while i < length:
        ...
        if i == 1:
            res['round_wind'] = ROUND_WIND[line[0]]  # ??
        elif 1 < i < 6:
            index = int(line[0])
            ...
            if op == 1:  # ??
                ...
                gang_tile = None if line[1] == '??' else line[2][2:-2]
...
def main():
    ...
    zip_root = 'output2017/'
    names = [
        # ('LIU', 12140)  # ??, uncomment if you want
        ('MO', 132994),  # ??
        ('PLAY', 385324)  # ??
    ]
